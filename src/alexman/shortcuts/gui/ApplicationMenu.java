...
class ApplicationMenu extends JMenuBar {
	...
	private final JMenu m_file, m_area, m_character, m_test;
	private final JMenuItem f_open, f_save, f_save_as, r_switch, c_add, c_remove, c_move, t_resize;
	private final Action a_open, a_save, a_save_as, a_switch, a_add, a_remove, a_move, a_resize;

	{
		...
		a_resize = new AbstractAction() {
			@Override
			public void actionPerformed(ActionEvent e) {
				context.reconsiderLifeChoices();
			}
		};
	}
	...
	private void listeners() {
		...
		t_resize.addActionListener(a_resize);
	}
	...
	private void mnemonics() {
		...
		m_test.setMnemonic('T');
	}

	private void accelerators() {
		f_open.setAccelerator(KeyStroke.getKeyStroke("control R")); // TODO: change to ctrl+O
		...
		t_resize.setAccelerator(KeyStroke.getKeyStroke("shift R"));
	}
